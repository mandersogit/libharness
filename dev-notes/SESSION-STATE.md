...
- **CRITICAL adversarial-review fixes landed (2026-05-25).** Nine adversarial reviews (2 Opus generalist + 7 codex-cli specialized) surfaced 3 CRITICAL bugs that are now fixed. **C1** (`close()` strands pending `_HookCall` / `_Continuation` items): `_dispatch_sync_hook` and `_register_continuation` now take `_close_lock` and complete the awaiting future with `RuntimeError` if the harness is closed. **C2** (sync hook on owner thread calling public API deadlocks queue): `submit()` detects owner-thread re-entry via `threading.get_ident()` and raises clear `RuntimeError`. **C3** (universal wheel ships host-specific pi binary): new `BdistWheelWithPiTag` cmdclass in `setup.py` tags wheels with the host platform when the binary is bundled. 7 new regression tests at `tests/pi/test_critical_review_fixes.py`. See `dev-notes/2026-05-25-adversarial-review-synthesis.md` for the full review + MODERATE backlog still pending.
- **F14 resolved: reader survives per-record decode errors; fails fast on catastrophic crashes (2026-05-25).** `StrictJsonlDecoder.feed()` now returns `(records, errors)` and skips bad records (state advances past them). Per-record errors are logged at ERROR, written to stderr (belt-and-braces), and counted on `PiRpcClient` (`_jsonl_decode_error_count` + `_first/_last_jsonl_decode_error`) so a pi-side bug is loud, not silent. For catastrophic reader exceptions (pipe broken, buffer overflow, framework bug), `_reader_failure` is stashed and `send()` fails fast with `PiRpcProcessError` instead of writing to pi's stdin and timing out 30s later. Regression: 5 cases in `test_jsonl.py` + 4 cases in `test_f14_reader_death.py`. Also fixed `server.py:_serve_client` for the new tuple return.
- **F8 resolved by folding sync hooks into the owner thread (Level 2, 2026-05-24).** `hook_executor` removed from `HarnessRuntime` entirely. Sync hooks (`on_*`, `decide_*`) enqueue `_HookCall` on the harness command queue; the owner thread (`PiAgentHarness-<id>`) consumes them serialized FIFO with the harness's other operations. Operations that touch the loop (`call_rpc`, `start`, `close`, `(un)subscribe_client_events`) now return `concurrent.futures.Future`; the owner-thread dispatcher registers a `_Continuation` callback and stays free to handle hook calls during the in-flight RPC.
- **Level 3 Option A: MainThread is the queue consumer in `threaded=False` (2026-05-25).** `_call` pumps the queue while waiting for its operation; `pump_until(coro_or_future_or_event)` lets tests that bypass the harness public API (white-box `_async_on_event` injection, raw bridge-socket I/O) pump explicitly. The `if self.threaded:` branch in `_dispatch_sync_hook` is gone — single dispatch code path; consumer varies only by *who* (dedicated owner thread under `threaded=True`, MainThread pump under `threaded=False`). One known limitation: blocking sync hooks under `threaded=False` block MainThread, so tests that need cancellation-during-blocking-hook should use `threaded=True` (documented in `docs/AGENT_HOOKS.md`). 4-thread model: MainThread / PiAsyncioLoop-Thread-2 / PiAgentHarness-`<id>` / pi-tool\_\*.
- **Pi distribution path implemented (2026-05-24).** libharness now vendors the precompiled pi binary from earendil-works's GitHub releases (pinned at v0.75.5, sha256-verified per platform). `src/libharness/_pi_vendor.py` is the canonical fetch + resolver module; `setup.py`'s `BuildPyWithPi` hook fires during wheel build (sdist user-install or CI wheel-build); `PiLaunchConfig.pi_command` defaults to `None` and defers to the resolver. Runtime resolver order: explicit `pi_command` → `LIBHARNESS_PI_PATH` env → vendored at `<package>/_vendor/pi/pi` → fail loud (NO PATH fallback). Dev loop: `make install-pi` runs the fetcher into `src/libharness/_vendor/pi/`; the legacy node-pi setup is preserved at `make install-pi-node-deprecated` until the binary path is thoroughly vetted. See `dev-notes/2026-05-24-pi-vendoring-design.md` (settled decisions + open Q1/Q3/Q4/Q5) and `dev-notes/2026-05-23-deno-as-pi-runtime-spike.md` (evidence trail).
...
- **Architecture:** asyncio core preserved but running in a dedicated shared loop thread (`HarnessRuntime.loop_thread`, process-wide singleton via `get_default_runtime()`). Each `PiAgentHarness` has its own owner thread that handles RPC dispatch AND sync hook execution (post-F8). Shared tool executor. No separate hook executor. The application main thread stays the application's.
- **Tests:** 157 unit tests + 3 live tests pass on 3.11 (155 + 3 live on 3.14t with 2 build-frontend-dependent C3 wheel-build tests skipped because FT venv lacks `build`). `make all`, `make test-live`, lint, mypy strict, pyright all clean. Test coverage: hooks dispatch (test_agent_class.py — including 3 strict-mode E2E tests for decision events from Item C), decision hooks (test_decision_hooks.py), channel separation (test_channel_separation.py), pi event taxonomy (test_pi_event_taxonomy.py — skips when pi-mono unavailable), threading model (test_threaded_agent.py), v5 regression (test_set_model.py, test_server.py, etc.), Phase 5.5 cherry-picks (test_v8_cherrypick_fixes.py — 6 cases), Gate A.5 Tier-1 fixes (test_gate_a5_tier1_fixes.py — 12 cases), pi vendoring (test_pi_vendor.py — 26 cases), JSONL decoder (test_jsonl.py — 5 cases), F14 reader-death (test_f14_reader_death.py — 4 cases).
...
- **`HarnessRuntime` scope audit** (from the Level 3 plan, threaded=False resolved 2026-05-25). Post-Level-2 + Option-A, `HarnessRuntime` owns only `loop_thread` + `tool_executor`. Open questions in `dev-notes/2026-05-24-level-3-runtime-audit.md`: Q1 naming, Q2 user-facing vs internal, Q3 singleton vs explicit construction, Q4 lifecycle. Not urgent — `HarnessRuntime`'s current shape works; the audit is for clarification before external-user-facing API freezes.
- **Tier-2 ergonomic pass** (the deferred-from-Gate-A.5 findings). 22 items in `dev-notes/2026-05-17-v8-port-review-synthesis.md` Tier-2 section; most are MODERATE/MINOR with mitigations or no current load-bearing impact. Highlights: F8 is now Done (2026-05-24, fold-hooks-into-owner-thread); `_call`/close race (F9), `_decision_timeouts_ms` mutable class default (F11), `_handler_wants_context` keyword-only edge case (F10 / Item D). No single-commit fix; revisit when an actual incident motivates the work.

...
- **Phase 3 of the threads rewrite landed.** `src/libharness/pi/rpc.py` is sync threaded (`subprocess.Popen` + reader threads + `dict[id, Future]` correlation + literal v3-synthesis UI ordering pattern). Async shims preserved on every public method via `asyncio.to_thread` so the still-asyncio harness + live tests stay green. New exception hierarchy: `PiRpcError` is now the abstract base; `PiRpcCommandError`, `PiRpcProcessError`, `ReentrantRPCError`, `EventQueueEmpty` are subclasses. 129/129 tests pass on both venvs; six consecutive runs, zero flakes. Three adversarial reviewers (2 codex gpt-5.5 xhigh + 1 Opus) found 1 CRITICAL + 7 HIGH bugs (dict-merge response key collision, close-deadlock behind blocked stdin, UI gate set after dispatch, etc.); all addressed before commit. See `dev-notes/2026-05-16-threads-rewrite-plan-phase3-review-synthesis.md` for the synthesis. **Phase 4 design proposal landed** at `dev-notes/2026-05-16-threads-rewrite-phase4-design.md` — pending adversarial design review, then implementation.

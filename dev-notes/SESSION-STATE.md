...
- **Phase 4 design v2 landed (pending second design review).** `dev-notes/2026-05-16-threads-rewrite-phase4-design.md` rewritten from v1 after 3 adversarial reviewers found 3 CRITICAL + multiple HIGH structural holes (`asyncio.CancelledError` MRO fact-error, `_start_complete` deadlock risk, `_cleanup()` race). v2 addresses every Tier-1 + Tier-2 finding: single-owner cleanup, explicit `start()` skeleton with `finally: _start_complete.set()`, cut-point reordering so non-unblockable steps come first, Python-based bisect-baseline guard, full test enumeration (17 lifecycle tests). Review synthesis: `dev-notes/2026-05-16-threads-rewrite-phase4-design-review-synthesis.md`. **Next: second 3-reviewer pass on v2; once converged, implementation begins.**
- **Phase 3 (`6c5a809`):** `src/libharness/pi/rpc.py` is sync threaded (`subprocess.Popen` + reader threads + `dict[id, Future]` correlation + literal v3-synthesis UI ordering). Async shims via `asyncio.to_thread`. New exception hierarchy: `PiRpcError` abstract base; `PiRpcCommandError`, `PiRpcProcessError`, `ReentrantRPCError`, `EventQueueEmpty` subclasses. 129/129 tests pass on both venvs; six consecutive runs, zero flakes. 3 reviewers found 1 CRITICAL + 7 HIGH bugs (dict-merge response key collision, close-deadlock behind blocked stdin, UI gate set after dispatch, etc.); all addressed before commit. See `dev-notes/2026-05-16-threads-rewrite-plan-phase3-review-synthesis.md`.
- **Phase 2 (`30c45608`):** `src/libharness/pi/server.py` is sync threaded (`socketserver.ThreadingTCPServer` + per-execute `asyncio.run` for async tool bodies). Async shims preserved on `PythonToolServer.start/close/__aenter__/__aexit__`. Three reviewers found ~11 must-fix bugs (CancelledError escape, partial-start hang, out-of-band race, etc.); all fixed before commit. See `dev-notes/2026-05-16-threads-rewrite-plan-phase2-review-synthesis.md`.

...
class PiAgentHarness:
    ...
    def close(self) -> None:
        ...
        try:
            ...
            if self.threaded:
                ...
                if self._owner_thread is not None and threading.get_ident() != self._owner_thread.ident:
    ...
    def send(self, command: dict[str, Any], *, timeout: float | None = None) -> dict[str, Any]:
        ...
        return self._call(lambda core: core.call_rpc("send", command, wait_timeout=wait_timeout, timeout=timeout))

    def prompt(self, message: str, *, timeout: float | None = None, **kwargs: Any) -> dict[str, Any]:
        return self._call(lambda core: core.call_rpc("prompt", message, wait_timeout=timeout, **kwargs))

    def prompt_and_wait(self, message: str, *, timeout: float = 120.0, **kwargs: Any) -> list[dict[str, Any]]:
        return self._call(lambda core: core.call_rpc("prompt_and_wait", message, wait_timeout=timeout + 1.0, timeout=timeout, **kwargs))

    def steer(self, message: str, *, timeout: float | None = None, **kwargs: Any) -> dict[str, Any]:
        return self._call(lambda core: core.call_rpc("steer", message, wait_timeout=timeout, **kwargs))

    def follow_up(self, message: str, *, timeout: float | None = None, **kwargs: Any) -> dict[str, Any]:
        return self._call(lambda core: core.call_rpc("follow_up", message, wait_timeout=timeout, **kwargs))
    ...
    def submit(self, operation: Callable[["_PiAgentHarnessCore"], Any]) -> concurrent.futures.Future[Any]:
    ...
    def _owner_loop(self) -> None:
        try:
            ...
            while True:
                ...
                if item is _STOP:
                    return
                operation, future = item  # type: ignore[misc]
...
class _PiAgentHarnessCore:
    ...
    def start(self) -> HarnessSnapshot:
        ...
        self.server = PythonToolServer(self.registry, tool_executor=self.runtime.tool_executor)
        self.endpoint = self.runtime.run_async(self.server.start())
        ...
        self.runtime.run_async(self.pi.start(extension_paths=extension_paths), timeout=max(10.0, config.startup_timeout + 5.0))
    ...
    def call_rpc(self, method_name: str, *args: Any, wait_timeout: float | None = None, **kwargs: Any) -> Any:
        self._check_owner()
        if self.pi is None:
    ...
    def _artifact_root(self) -> Path:
        ...
        if self._tempdir is None:
            self._tempdir = tempfile.TemporaryDirectory(prefix=f"pi-agent-harness-{self.harness_id}-")

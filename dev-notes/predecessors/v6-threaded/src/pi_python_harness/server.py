...
from .tools import ToolContext, ToolError, ToolRegistry, collect_tool_result, exception_to_wire, normalize_tool_value
...
class PythonToolServer:
    ...
    async def _dispatch_execute(self, request: dict[str, Any], reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            ...
            if self.tool_executor is None:
                ...
                result = await loop.run_in_executor(self.tool_executor, self._execute_tool_blocking, tool_name, params, ctx)
    ...
    async def _execute_tool_async(self, tool_name: str, params: dict[str, Any], ctx: ToolContext) -> Any:
        registered = self.registry.get(tool_name)
        raw = registered.call(params, ctx)
        return await collect_tool_result(raw, ctx)

    def _execute_tool_blocking(self, tool_name: str, params: dict[str, Any], ctx: ToolContext) -> Any:
        registered = self.registry.get(tool_name)
        raw = registered.call(params, ctx)

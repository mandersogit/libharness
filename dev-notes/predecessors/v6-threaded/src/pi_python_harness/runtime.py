...
class AsyncioLoopThread:
    ...
    def __init__(self, *, thread_name: str = "PiAsyncioLoop", daemon: bool = True) -> None:
    ...
    def start(self) -> "AsyncioLoopThread":
        with self._start_lock:
            ...
            self._ready.clear()
            self._stopped.clear()
            self._thread = threading.Thread(target=self._run, name=self.thread_name, daemon=self.daemon)
            self._thread.start()
        self._ready.wait()
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            except AttributeError:  # pragma: no cover - Python < 3.9 compatibility path
                pass
...
class HarnessRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        loop_thread_name: str = "PiAsyncioLoop",
    ...
    def close(self, *, shutdown_executor: bool = True, cancel_futures: bool = True, timeout: float = 5.0) -> None:
        if self._closed:
            return

...
2. **Decision hooks** participate in Pi's in-process extension event surface. Their return values are
   handed back to Pi.

```python
from pi_python_harness import Agent, AgentEvent, HookContext
...
| Event | Expected result shape | Example return |
|---|---|---|
...
| `session_before_tree` | `{cancel?, summary?, customInstructions?, replaceInstructions?, label?}` | `{"cancel": False, "label": "reviewed"}` |
...
| `tool_call` | `{block?, reason?}`; mutate `event.input` in TS to modify args, but Python returns only the result shape. | `{"block": True, "reason": "blocked by policy"}` |
...
| `input` | `{action:"continue"}` or `{action:"transform", text, images?}` or `{action:"handled"}` | `{"action": "transform", "text": rewritten}` |

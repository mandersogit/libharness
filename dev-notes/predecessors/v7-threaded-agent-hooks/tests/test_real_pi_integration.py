...
@pytest.mark.skipif(
    not os.environ.get("PI_CLI"),
    reason="set PI_CLI to a Pi CLI executable or dist/cli.js for real integration",
)
async def test_real_pi_runs_bridge_and_python_tool_loop() -> None:
    pi_cli = Path(os.environ["PI_CLI"])
    command = (
        [sys.executable, str(pi_cli)]
        if pi_cli.suffix == ".py"
        else (["node", str(pi_cli)] if pi_cli.suffix == ".js" else [str(pi_cli)])
    )
    ...
    config = PiLaunchConfig(
        pi_command=command, request_timeout=90, startup_timeout=5, no_builtin_tools=True
    )

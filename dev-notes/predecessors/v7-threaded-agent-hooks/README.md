...
`Agent` inherits from `PiAgentHarness` and adds named event hooks while keeping the
...
```python
from pi_python_harness import Agent, AgentEvent, ToolRegistry, ToolResult
...
class MyAgent(Agent):
    async def async_on_agent_start(self, event: AgentEvent) -> None:
        print(f"started: {event.type}")

    def on_tool_execution_end(self, event: AgentEvent) -> None:
        print(event["toolName"], event["isError"])
...
See `docs/AGENT_HOOKS.md` for the event table, strict-mode behavior, lifecycle
wiring, and the hook executor rationale.

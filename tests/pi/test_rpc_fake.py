...
async def test_rpc_client_handles_headless_ui_and_events() -> None:
    ...
    client = PiRpcClient(PiLaunchConfig(pi_command=[sys.executable, str(script)], request_timeout=5, offline=False, no_extensions=False, no_skills=False, no_prompt_templates=False, no_context_files=False))
    async with client:

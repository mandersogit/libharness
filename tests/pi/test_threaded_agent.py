...
from libharness.pi import HarnessRuntime, PiAgentHarness, PiLaunchConfig, ToolRegistry
...
def test_runtime_loop_thread_is_not_main_thread() -> None:
    runtime = HarnessRuntime(
        loop_thread_name="PiAsyncioLoop-Thread-2", tool_thread_name_prefix="test-pi-tool"
    )
    try:
...
def test_one_harness_can_live_on_main_thread_for_tests() -> None:
    runtime = HarnessRuntime(
        loop_thread_name="PiAsyncioLoop-Thread-2-test-main", tool_thread_name_prefix="test-pi-tool"
    )
...
def test_multiple_harnesses_have_distinct_owner_threads_and_shared_loop() -> None:
    runtime = HarnessRuntime(
        loop_thread_name="PiAsyncioLoop-Thread-2-shared", tool_thread_name_prefix="shared-pi-tool"
    )
    h1 = PiAgentHarness(
        ToolRegistry(),
        config=fake_config(),
        runtime=runtime,
        owner_thread_name="PiAgentHarness-one",
    )
    h2 = PiAgentHarness(
        ToolRegistry(),
        config=fake_config(),
        runtime=runtime,
        owner_thread_name="PiAgentHarness-two",
    )
    try:
        h1.start()
        h2.start()
...
async def _execute(endpoint, tool: str, params: dict) -> dict:
    ...
    writer.write(
        dumps_line(
            {"id": tool, "type": "execute", "token": endpoint.token, "tool": tool, "params": params}
        )
    )
    await writer.drain()
...
async def test_tool_calls_for_multiple_registries_use_shared_threadpool() -> None:
    runtime = HarnessRuntime(
        loop_thread_name="PiAsyncioLoop-Thread-2-tools", tool_thread_name_prefix="shared-pi-tool"
    )
